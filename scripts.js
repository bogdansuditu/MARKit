...
function syncScroll(source, target, sourceIsEditor = true) {
  ...
  const sourceInfo = sourceIsEditor
    ? source.getScrollInfo()
    : {
  ...
  if (
    (sourceIsEditor && isPreviewScrolling) ||
    (!sourceIsEditor && isEditorScrolling)
  ) {
    return;
  }
...
function extractTitle(content) {
  ...
  title = title
    .replace(/[^a-zA-Z0-9\s-_.]/g, "") // Keep only alphanumeric, spaces, hyphens, dots
  ...
  fetch("file_operations.php", {
    ...
    .then((data) => {
      ...
      sortedItems.forEach((item) => {
        ...
        icon.className =
          item.type === "directory" ? "fas fa-folder" : "fas fa-file-alt";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  style.textContent = `
        ...
        .CodeMirror .cm-header { color: var(--text-color); font-weight: bold; }
        .CodeMirror .cm-quote { color: var(--text-color); font-style: italic; }
        .CodeMirror .cm-link { color: var(--text-color); }
        .CodeMirror .cm-url { color: var(--text-color); }
        .CodeMirror .cm-string { color: var(--text-color); }
        .CodeMirror span.cm-variable-2 { color: var(--text-color) !important; }
        .CodeMirror span.cm-variable-3 { color: var(--text-color) !important; }
        .CodeMirror span.cm-variable-4 { color: var(--text-color) !important; }
        .CodeMirror-gutters {
  ...
  if (recentFilesBtn && recentFilesDropdown) {
    ...
    document.addEventListener("click", (e) => {
      if (
        !recentFilesDropdown.contains(e.target) &&
        !recentFilesBtn.contains(e.target)
      ) {
        ...
        if (result.isConfirmed) {
          ...
          fetch("file_operations.php", {
            ...
            .catch((error) => {
              ...
              Swal.fire(
                "Error",
                "Failed to reset application: " + error.message,
                "error",
              );
            });
        }
          ...
          document
            ...
            .forEach((item) => {
              item.addEventListener("click", () => {
                ...
                const folderName =
                  item.querySelector(".folder-name").textContent;
                document.querySelector(".selected-folder-name").textContent =
                  folderName;
              });
            });
        },
...
function buildFolderTreeHtml(folders, currentFolderId) {
  let html =
    '<div class="file-list-container"><ul id="file-list" class="list-group">';
  ...
  folders.forEach((folder) => {
    if (folder.parent_folderid && folderMap.has(folder.parent_folderid)) {
      folderMap
        .get(folder.parent_folderid)
        .children.push(folderMap.get(folder.folderid));
    }
  });

  // Get root folders (parent_folderid === null)
  const rootFolders = Array.from(folderMap.values()).filter(
    (folder) => folder.parent_folderid === null,
  );
...
function saveDocument(forcePath = null) {
  return new Promise((resolve, reject) => {
    ...
    const title = documentState.isNewDocument
      ...
      : documentState.currentNoteId
        ? documentState.lastSavedTitle
        : extractTitle(content);
...
async function loadRecentModifiedFiles() {
  try {
    ...
    data.files.forEach((file) => {
      ...
      timeSpan.textContent = date.toLocaleDateString(undefined, {
        year: "numeric",
        month: "short",
        day: "numeric",
        hour: "2-digit",
...
async function exportToPDF() {
  ...
  for (let el of elements) {
    ...
    if (
      ...
      (el.classList.contains("language-") ||
        Array.from(el.classList).some((cls) => cls.startsWith("language-")))
    ) {
  ...
  const codeToolbars = contentClone.querySelectorAll(
...
function initializeSearch() {
  ...
  searchTypeBtn.addEventListener("click", (e) => {
    ...
    searchDropdown.style.display =
      searchDropdown.style.display === "none" ? "block" : "none";
  });
...
function displaySearchResults(results) {
  ...
  if (results.length === 0) {
    ...
    noResults.innerHTML =
      '<i class="fas fa-search"></i><span>No results found</span>';
...
async function updateStatusBar() {
  ...
  try {
    ...
    if (documentState.lastSavedTitle) {
      pathEl.innerHTML +=
        '<span class="separator">/</span>' + documentState.lastSavedTitle;
    }
